            ...
            switch (strings[0]) {
                ...
                case "play" -> { // dialogue play BizarreDark test.yml
                    if (commandSender.isPlayer() && !commandSender.isOp()) {
                        commandSender.sendMessage(DialogueMain.getLanguage().translateString(commandSender.asPlayer(), "command_not_authorized"));
                        return true;
                    }
                    if (strings.length == 3) {
                        ...
                        if (DialogueMain.getDialogues().containsKey(fileName)) {
                            ...
                            if (player != null) {
                                if (DialogueMain.playerPlayingTasks.containsKey(player)) { // 避免玩家已经播放对话
                                    if (isPlayer) {
                                        commandSender.sendMessage(DialogueMain.getLanguage().translateString(commandSender.asPlayer(), "command_player_in_dialogue", playerName));
                                    }
                                    return true;
                                }
                                ...
                                if (isPlayer) {
                                    commandSender.sendMessage(DialogueMain.getLanguage().translateString(commandSender.asPlayer(), "command_player_not_found", playerName));
                                }
                            }
                        } else {
                            if (isPlayer) {
                                commandSender.sendMessage(DialogueMain.getLanguage().translateString(commandSender.asPlayer(), "command_dialogue_not_found", fileName));
                            }
                        }
                    }else{
                        Server.getInstance().executeCommand(commandSender, "dialogue help");
                ...
                case "edit" -> {
                    if(commandSender.isOp()){
                        FormHelper.showDialogueSelect(commandSender.asPlayer());
